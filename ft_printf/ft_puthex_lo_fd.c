...
/*   Updated: 2024/09/23 11:04:04 by akiss            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <unistd.h>

void	ft_buffer_number_lo(unsigned long long number,
int radix, int buffer[], int index)
{
	if (number > (unsigned long long)(radix - 1))
		ft_buffer_number_lo
		(number / radix, radix, buffer, index + 1);
...
void	ft_puthex_lo_fd(unsigned long long number, int fd)
{
	...
	ft_buffer_number_lo(number, 16, buffer, 0);
	index = 15;
